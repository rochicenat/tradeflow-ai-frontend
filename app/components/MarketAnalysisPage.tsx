  ...
  const fetchStockData = async () => {
    ...
    try {
      const promises = stocks.map(stock =>
        fetch(`https://finnhub.io/api/v1/quote?symbol=${stock.symbol}&token=A8YPFV9V3FT03VZP`)
          .then(r => r.json())
          .then(data => ({
            symbol: stock.symbol,
            ...
            volume: `${((data.v || 0) / 1e6).toFixed(1)}M`
          }))
      );
  ...
  const fetchForexData = async () => {
    try {
      const pairs = [
        ...
        { from: 'USD', to: 'TRY', display: 'USD/TRY' }
      ];
      ...
      const promises = pairs.map(async pair => {
        ...
        return {
          ...
          volume: pair.to === 'TRY' ? '$12.4B' : pair.to === 'JPY' ? '$1.3T' : '$2.1T'
        };
      });
    ...
    <div className="max-w-7xl">
      ...
      <div className="bg-blue-500/10 border border-orange-500/30 rounded-xl p-4 mb-8 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-slate-300 text-sm">
            All data updates automatically every 60 seconds. For educational and research purposes only.
          </p>
        </div>
      </div>
      ...
      <div className="mb-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="px-3 py-1 bg-green-500/20 text-green-400 text-xs font-semibold rounded-full animate-pulse">LIVE</span>
        </div>
      ...
      <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-slate-300 text-sm">
              Live data from CoinGecko (crypto), Finnhub (stocks), ExchangeRate-API (forex). For educational purposes only. Not financial advice.
            </p>
          </div>
        </div>
      </div>
